...
export default function SpeciesDetailPanel({ species, onClose, onStartProgram }) {
  ...
  return (
    <>
      ...
      <div style={{ position:"fixed", top:0, right:0, bottom:0, width:540, maxWidth:"96vw", background:"#fff", zIndex:101, display:"flex", flexDirection:"column", boxShadow:"-4px 0 24px rgba(0,0,0,0.12)" }}>
        ...
        {(species.composite_score || species.score_conservation) && (
          <div style={{ padding:"10px 20px", borderBottom:"1px solid #e8e6e1", display:"flex", gap:6, flexShrink:0 }}>
            {[
              ...
              <div key={m.l} style={{ flex:1, background:"#f4f3ef", borderRadius:8, padding:"6px 8px", textAlign:"center" }}>
        ...
        <div style={{ padding:"10px 20px", borderBottom:"1px solid #e8e6e1", flexShrink:0, display:"flex", alignItems:"center", justifyContent:"space-between", gap:8 }}>
          ...
          <button onClick={() => onStartProgram && onStartProgram(species)} style={{ padding:"6px 14px", border:"none", borderRadius:8, background:"#1D9E75", color:"#fff", fontSize:11, fontWeight:600, cursor:"pointer" }}>
            + Start Program
          </button>
        </div>
        ...
        <div style={{ display:"flex", borderBottom:"1px solid #e8e6e1", flexShrink:0, overflowX:"auto" }}>
          {TABS.map(t => (
            <button key={t.k} onClick={() => setTab(t.k)} style={{ flexShrink:0, padding:"10px 12px", border:"none", borderBottom:tab===t.k?"2px solid #1D9E75":"2px solid transparent", background:"none", cursor:"pointer", fontSize:11, fontWeight:tab===t.k?600:400, color:tab===t.k?"#1D9E75":"#888", whiteSpace:"nowrap" }}>
              {t.l}
            </button>
          ))}
        </div>
          ...
          {loading ? (
            ...
            <>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                  ...
                  <div style={{ padding:"14px 16px", background:"#fff", borderRadius:12, border:"1px solid #e8e6e1" }}>
                    ...
                    <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                      {[
                        { l:"Propagation Protocol", status: prop.length > 0 ? "strong" : (species.tc_status && species.tc_status !== "Not established") ? "partial" : "missing", note: prop.length > 0 ? `${prop.length} protocol(s)` : species.tc_status || "No data" },
                        { l:"Metabolite Evidence",  status: mets.length > 5 ? "strong" : mets.length > 0 ? "partial" : "missing", note: mets.length > 0 ? `${mets.length} compounds` : "No data" },
                        { l:"Conservation Assessment", status: cons.length > 0 ? "strong" : (species.iucn_status && species.iucn_status !== "NE") ? "partial" : "missing", note: cons.length > 0 ? cons[0].source : species.iucn_status || "Not evaluated" },
                        { l:"Commercial Hypothesis", status: comm.length > 0 ? "strong" : species.market_area ? "partial" : "missing", note: comm.length > 0 ? comm[0].application_area : species.market_area || "No hypothesis" },
                        { l:"Governance Readiness",  status: gov ? (gov.abs_nagoya_risk === "low" ? "strong" : "partial") : "missing", note: gov ? `ABS risk: ${gov.abs_nagoya_risk || "unknown"}` : "Not assessed" },
                        { l:"Field / Location Data", status: locs.length > 2 ? "strong" : locs.length > 0 ? "partial" : "missing", note: locs.length > 0 ? `${locs.length} location(s)` : "No data" },
                        { l:"Publications",          status: pubs.length > 10 ? "strong" : pubs.length > 0 ? "partial" : "missing", note: pubs.length > 0 ? `${pubs.length} publications` : "No publications" },
                      ].map(gap => {
                  ...
                  <div style={{ padding:"14px 16px", background:"#fff", borderRadius:12, border:"1px solid #e8e6e1" }}>
                    ...
                    <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                      {[
                        ...
                        return (
                          <div key={i} style={{ display:"flex", alignItems:"flex-start", gap:10, padding:"9px 10px", background:"#f8f7f4", borderRadius:8, borderLeft:`3px solid ${uc[item.urgency]}` }}>
                            <span style={{ fontSize:9, padding:"2px 6px", borderRadius:99, background:ub[item.urgency], color:uc[item.urgency], fontWeight:700, flexShrink:0, marginTop:1 }}>{item.urgency.toUpperCase()}</span>
                            <div>
                      ...
                      {[prop.length === 0, mets.length === 0, locs.length === 0, comm.length === 0, !gov, !story].every(x => !x) && (
                        <div style={{ textAlign:"center", padding:20, color:"#888", fontSize:12 }}>✅ All key data points are covered</div>
                      )}
                    </div>
                  </div>
                  ...
                  <div style={{ padding:"14px 16px", background:"#fff", borderRadius:12, border:"1px solid #e8e6e1" }}>
                    ...
                    <div style={{ padding:"12px 14px", background:"#f8f7f4", borderRadius:10, border:"1px solid #e8e6e1" }}>
                      {species.recommended_pathway && (
                        <div style={{ fontSize:11, color:"#5f5e5a", marginBottom:10 }}>Suggested pathway: <strong>{species.recommended_pathway}</strong></div>
                      )}
                      <button onClick={() => onStartProgram && onStartProgram(species)} style={{ width:"100%", padding:10, background:"#1D9E75", color:"#fff", border:"none", borderRadius:8, fontSize:12, fontWeight:700, cursor:"pointer" }}>
                        + Start Program
                      </button>
                    </div>
                  </div>
              ...
              {tab === "story" && (!story ? (
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                  <StoryBlock color="#1D9E75" bg="linear-gradient(135deg,#E1F5EE,#f8fff8)" border="1px solid #1D9E75" label="GEOCON Perspective">
                    {story.geocon_rationale && <p style={{ fontSize:12, color:"#2c2c2a", lineHeight:1.7, margin:"0 0 8px" }}>{story.geocon_rationale}</p>}
                    {story.rescue_urgency && <div style={{ fontSize:11, color:"#A32D2D", lineHeight:1.6, padding:"8px 10px", background:"#FCEBEB", borderRadius:8 }}><strong style={{ fontSize:9, textTransform:"uppercase" }}>Rescue urgency: </strong>{story.rescue_urgency}</div>}
                  </StoryBlock>
                  <StoryBlock color="#534AB7" bg="#f8f7f4" border="1px solid #e8e6e1" label="Scientific narrative">
                    {story.scientific_narrative && <p style={{ fontSize:12, color:"#2c2c2a", lineHeight:1.7, margin:"0 0 10px" }}>{story.scientific_narrative}</p>}
                    {story.habitat_story && <><div style={{ fontSize:9, color:"#888", textTransform:"uppercase", marginBottom:4 }}>Habitat</div><p style={{ fontSize:11, color:"#5f5e5a", lineHeight:1.6, margin:0 }}>{story.habitat_story}</p></>}
                  </StoryBlock>
                  {story.conservation_context && <StoryBlock color="#BA7517" bg="#FAEEDA" border="1px solid #BA7517" label="Conservation context"><p style={{ fontSize:12, color:"#2c2c2a", lineHeight:1.7, margin:0 }}>{story.conservation_context}</p></StoryBlock>}
                  {story.propagation_pathway && <StoryBlock color="#085041" bg="#E1F5EE" border="1px solid #1D9E75" label="Propagation pathway"><p style={{ fontSize:12, color:"#2c2c2a", lineHeight:1.7, margin:0 }}>{story.propagation_pathway}</p></StoryBlock>}
                  <div style={{ padding:"14px 16px", background:"#f8f7f4", borderRadius:12, border:"1px solid #e8e6e1", borderLeft:"3px solid #185FA5" }}>
                    ...
                    {story.commercial_hypothesis && <p style={{ fontSize:12, color:"#2c2c2a", lineHeight:1.7, margin:"0 0 8px" }}>{story.commercial_hypothesis}</p>}
                    {story.market_narrative && <p style={{ fontSize:11, color:"#5f5e5a", lineHeight:1.6, margin:"0 0 6px" }}><strong style={{ fontSize:9, textTransform:"uppercase", color:"#888" }}>Market: </strong>{story.market_narrative}</p>}
                    {story.value_chain && <p style={{ fontSize:11, color:"#5f5e5a", lineHeight:1.6, margin:0 }}><strong style={{ fontSize:9, textTransform:"uppercase", color:"#888" }}>Value chain: </strong>{story.value_chain}</p>}
                  </div>
              ...
              {/* ══ PUBLICATIONS TAB ══ */}
              {tab === "pubs" && (pubs.length === 0 ? (
                <p style={{ color:"#999", fontSize:13, textAlign:"center", padding:20 }}>No publications found</p>
              ) : pubs.map(p => (
                <div key={p.id} style={{ marginBottom:10, padding:"10px 12px", background:"#f8f7f4", borderRadius:8, borderLeft:"3px solid #378ADD" }}>
              ...
              {/* ══ METABOLITES TAB ══ */}
              {tab === "mets" && (mets.length === 0 ? (
                <p style={{ color:"#999", fontSize:13, textAlign:"center", padding:20 }}>No metabolites yet</p>
              ) : mets.map(m => (
                <div key={m.id} style={{ marginBottom:10, padding:"10px 12px", background:"#f8f7f4", borderRadius:8, borderLeft:"3px solid #534AB7" }}>
              ...
              {/* ══ CONSERVATION TAB ══ */}
              {tab === "cons" && (cons.length === 0 ? (
                <div style={{ textAlign:"center", padding:32 }}><p style={{ color:"#999", fontSize:13 }}>No conservation assessments yet</p></div>
              ) : cons.map(a => (
              ...
              {/* ══ GOVERNANCE TAB ══ */}
              {tab === "gov" && (!gov ? (
                <div style={{ textAlign:"center", padding:32 }}><p style={{ color:"#999", fontSize:13 }}>No governance data yet</p></div>
              ) : (
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px 16px" }}>
                      {[
                        ...
                        <div key={l}>
                          ...
                          {col ? <span style={{ fontSize:11, padding:"2px 8px", borderRadius:99, background:riskBg(v), color:riskColor(v), fontWeight:600 }}>{v}</span> : <div style={{ fontSize:12, color:"#2c2c2a", fontWeight:500 }}>{v}</div>}
                        </div>
                      ) : null)}
                    </div>
                  </div>
              ...
              {/* ══ PROPAGATION TAB ══ */}
              {tab === "prop" && (prop.length === 0 ? (
                <div style={{ textAlign:"center", padding:32 }}><p style={{ color:"#999", fontSize:13 }}>No propagation protocols yet</p></div>
              ) : prop.map(p => (
              ...
              {/* ══ COMMERCIAL TAB ══ */}
              {tab === "comm" && (comm.length === 0 ? (
                <div style={{ textAlign:"center", padding:32 }}><p style={{ color:"#999", fontSize:13 }}>No commercial hypotheses yet</p></div>
              ) : comm.map(h => (
              ...
              {tab === "info" && (
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px 16px" }}>
                  {[
                    ...
                    <div key={l}>
                      <div style={{ fontSize:9, color:"#b4b2a9", textTransform:"uppercase", letterSpacing:0.4 }}>{l}</div>
...
function StoryBlock({ color, bg, border, label, children }) {
  return (
    <div style={{ padding:"14px 16px", background:bg, borderRadius:12, border }}>
      <div style={{ fontSize:9, color, textTransform:"uppercase", letterSpacing:0.8, fontWeight:600, marginBottom:8 }}>{label}</div>
      {children}
    </div>
  );
}
