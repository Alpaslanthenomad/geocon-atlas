...
function SpeciesModule({species,exp,setExp,onSpeciesClick}){
  const[search,setSearch]=useState("");const[fC,setFC]=useState("all");const[fD,setFD]=useState("all");const[sort,setSort]=useState("composite");const[viewMode,setViewMode]=useState("score");const[famSearch,setFamSearch]=useState("");const[famFilter,setFamFilter]=useState("all");
  ...
  const famFiltered=species.filter(s=>{const ms=!famSearch||s.accepted_name?.toLowerCase().includes(famSearch.toLowerCase());const mf=famFilter==="all"||s.family===famFilter;return ms&&mf;});
  const grouped=families.reduce((acc,fam)=>{const members=famFiltered.filter(s=>s.family===fam);if(members.length>0)acc[fam]=members;return acc;},{});
  return<>
    <div style={{display:"flex",gap:6,marginBottom:12,alignItems:"center",flexWrap:"wrap"}}>
      <div style={{display:"flex",border:"1px solid #e8e6e1",borderRadius:8,overflow:"hidden",marginRight:8}}>{[{k:"score",l:"Score View"},{k:"family",l:"Family View"}].map(v=><button key={v.k} onClick={()=>setViewMode(v.k)} style={{padding:"6px 14px",fontSize:11,border:"none",borderRight:v.k==="score"?"1px solid #e8e6e1":"none",cursor:"pointer",background:viewMode===v.k?"#f4f3ef":"#fff",color:viewMode===v.k?"#2c2c2a":"#888",fontWeight:viewMode===v.k?600:400}}>{v.l}</button>)}</div>
      {viewMode==="score"&&<><input type="text" placeholder="Search..." value={search} onChange={e=>setSearch(e.target.value)} style={{flex:"1 1 140px",minWidth:120,...S.input}}/><select value={fC} onChange={e=>setFC(e.target.value)} style={S.input}><option value="all">All countries</option>{countries.map(c=><option key={c} value={c}>{c==="TR"?"Türkiye":"Chile"}</option>)}</select><select value={fD} onChange={e=>setFD(e.target.value)} style={S.input}><option value="all">All decisions</option>{decisions.map(d=><option key={d} value={d}>{d}</option>)}</select><select value={sort} onChange={e=>setSort(e.target.value)} style={S.input}><option value="composite">Composite</option><option value="conservation">Conservation</option><option value="venture">Venture</option><option value="trl">TRL</option><option value="name">Name</option></select></>}
      {viewMode==="family"&&<><input type="text" placeholder="Tür ara..." value={famSearch} onChange={e=>setFamSearch(e.target.value)} style={{flex:"1 1 160px",minWidth:140,...S.input}}/><select value={famFilter} onChange={e=>setFamFilter(e.target.value)} style={S.input}><option value="all">Tüm familyalar</option>{families.map(f=><option key={f} value={f}>{f} ({species.filter(s=>s.family===f).length})</option>)}</select></>}
    </div>
    {viewMode==="score"&&<><p style={{...S.sub,margin:"0 0 8px"}}>{filtered.length}/{species.length} species</p><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))",gap:8}}>{filtered.map(sp=><SpeciesCard key={sp.id} sp={sp} expanded={exp===sp.id} onToggle={()=>setExp(exp===sp.id?null:sp.id)} onDetailClick={()=>onSpeciesClick(sp)}/>)}</div></>}
    {viewMode==="family"&&<><div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}><div style={{background:"#f4f3ef",borderRadius:8,padding:"6px 12px",fontSize:12}}><span style={{color:"#999"}}>Gösterilen: </span><strong>{famFiltered.length}</strong></div><div style={{background:"#f4f3ef",borderRadius:8,padding:"6px 12px",fontSize:12}}><span style={{color:"#999"}}>Familya: </span><strong>{Object.keys(grouped).length}</strong></div></div>{Object.entries(grouped).map(([fam,spp],i)=><FamilyGroup key={fam} family={fam} species={spp} defaultOpen={i<3} onSpeciesClick={onSpeciesClick}/>)}{famFiltered.length===0&&<p style={{textAlign:"center",padding:40,color:"#999"}}>Sonuç bulunamadı</p>}</>}
  </>
}
...
function PublicationsView({publications}){
  const[search,setSearch]=useState("");const[expanded,setExpanded]=useState(null);const[page,setPage]=useState(0);const[sourceFilter,setSourceFilter]=useState("all");const[yearFilter,setYearFilter]=useState("all");
  const PAGE_SIZE=50;
  ...
  const filtered=publications.filter(p=>{if(search){const s=search.toLowerCase();if(!(p.title||"").toLowerCase().includes(s)&&!(p.authors||"").toLowerCase().includes(s)&&!(p.journal||"").toLowerCase().includes(s)&&!(p.species?.accepted_name||"").toLowerCase().includes(s))return false;}if(sourceFilter!=="all"&&p.source!==sourceFilter)return false;if(yearFilter!=="all"&&(!p.year||Math.floor(p.year/10)*10!==parseInt(yearFilter)))return false;return true;});
  ...
  return<div>
    <div style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap"}}>
      <input type="text" placeholder="Başlık, yazar, dergi, tür ara..." value={search} onChange={e=>{setSearch(e.target.value);setPage(0);}} style={{flex:"1 1 200px",minWidth:160,...S.input}}/>
      <select value={sourceFilter} onChange={e=>{setSourceFilter(e.target.value);setPage(0);}} style={S.input}><option value="all">Tüm kaynaklar</option>{sources.map(s=><option key={s} value={s}>{s} ({publications.filter(p=>p.source===s).length})</option>)}</select>
      <select value={yearFilter} onChange={e=>{setYearFilter(e.target.value);setPage(0);}} style={S.input}><option value="all">Tüm yıllar</option>{decades.map(d=><option key={d} value={d}>{d}s</option>)}</select>
    </div>
    <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}>{[{l:"Toplam",v:publications.length},{l:"Filtrelenen",v:filtered.length},{l:"Open Access",v:oaCount},{l:"Kaynak",v:sources.length},{l:"Tür",v:[...new Set(publications.map(p=>p.species_id).filter(Boolean))].length}].map(s=><div key={s.l} style={{flex:"1 1 80px",...S.metric,padding:"6px 10px"}}><div style={S.mLabel}>{s.l}</div><div style={{...S.mVal(),fontSize:16}}>{s.v}</div></div>)}</div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))",gap:8}}>
      {paginated.map(p=><div key={p.id} onClick={()=>setExpanded(expanded===p.id?null:p.id)} style={{...S.card,padding:14,cursor:"pointer",border:expanded===p.id?"2px solid #85B7EB":"1px solid #e8e6e1"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,marginBottom:4}}><div style={{flex:1}}><div style={{fontSize:12,fontWeight:600,color:"#2c2c2a",lineHeight:1.4}}>{(p.title||"Untitled").slice(0,120)}{(p.title||"").length>120?"...":""}</div></div><div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:2,flexShrink:0}}>{p.year&&<Pill color="#0C447C" bg="#E6F1FB">{p.year}</Pill>}{p.open_access&&<Pill color="#085041" bg="#E1F5EE">OA</Pill>}</div></div>
        <div style={{fontSize:10,color:"#888",marginBottom:4}}>{(p.authors||"—").slice(0,80)}{(p.authors||"").length>80?"...":""}</div>
        <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{p.source&&<Pill color="#712B13" bg="#FAECE7">{p.source}</Pill>}{p.journal&&<Pill color="#3C3489" bg="#EEEDFE">{(p.journal||"").slice(0,30)}</Pill>}{p.species?.accepted_name&&<Pill color="#085041" bg="#E1F5EE">{p.species.accepted_name}</Pill>}{p.primary_topic&&<Pill color="#854F0B" bg="#FAEEDA">{(p.primary_topic||"").slice(0,25)}</Pill>}</div>
        {expanded===p.id&&<div style={{marginTop:10,paddingTop:10,borderTop:"1px solid #e8e6e1",fontSize:11}}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"6px 14px"}}>{[{l:"DOI",v:p.doi?p.doi.replace("https://doi.org/",""):""},{l:"Kaynak",v:p.source},{l:"Konu",v:p.primary_topic},{l:"Atıf",v:p.cited_by_count||"—"}].map(({l,v})=><div key={l}><span style={{color:"#b4b2a9",fontSize:9}}>{l}</span><div style={{color:"#2c2c2a"}}>{v||"—"}</div></div>)}</div>{p.abstract&&<div style={{marginTop:8,fontSize:10,color:"#5f5e5a",lineHeight:1.5,borderTop:"1px solid #e8e6e1",paddingTop:8}}>{p.abstract.slice(0,400)}...</div>}{p.doi&&<a href={p.doi} target="_blank" rel="noopener noreferrer" style={{display:"inline-block",marginTop:6,fontSize:10,color:"#185FA5",textDecoration:"none"}}>Makaleyi görüntüle ↗</a>}</div>}
      </div>)}
    </div>
