...
/* ─── FAMILY CARD (simple) ─── */
function FamilySpeciesCard({sp}){
  const c=FAMILY_COLORS[sp.family]||DEF_FAM;
  return<div style={{background:"#fff",border:"0.5px solid #e8e6e1",borderLeft:`3px solid ${c.dot}`,borderRadius:10,padding:"10px 12px"}}>
...
function FamilyGroup({family,species,defaultOpen}){
  ...
  return<div style={{marginBottom:10}}>
    <button onClick={()=>setOpen(!open)} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",background:c.bg,border:`0.5px solid ${c.border}`,borderRadius:open?"10px 10px 0 0":"10px",padding:"9px 14px",cursor:"pointer"}}>
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        <span style={{width:8,height:8,borderRadius:"50%",background:c.dot,display:"inline-block",flexShrink:0}}/>
        <span style={{fontSize:13,fontWeight:600,color:c.text}}>{family}</span>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        <span style={{fontSize:11,color:c.text,opacity:0.7}}>{species.length} tür</span>
        <span style={{fontSize:11,color:c.text,opacity:0.5}}>{open?"▲":"▼"}</span>
      </div>
    </button>
    {open&&<div style={{border:`0.5px solid ${c.border}`,borderTop:"none",borderRadius:"0 0 10px 10px",padding:10,display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:8,background:"#f8f7f4"}}>
      {species.map(s=><FamilySpeciesCard key={s.id} sp={s}/>)}
    </div>}
  </div>
}

/* ─── SPECIES MODULE (combined score + family view) ─── */
function SpeciesModule({species,exp,setExp}){
  const[search,setSearch]=useState("");
  const[fC,setFC]=useState("all");
  const[fD,setFD]=useState("all");
  const[sort,setSort]=useState("composite");
  const[viewMode,setViewMode]=useState("score"); // "score" | "family"
  ...
  const filtered=species.filter(s=>{
    if(search&&!(s.accepted_name||"").toLowerCase().includes(search.toLowerCase())&&!(s.genus||"").toLowerCase().includes(search.toLowerCase()))return false;
    if(fC!=="all"&&s.country_focus!==fC)return false;
    if(fD!=="all"&&s.decision!==fD)return false;
    return true;
  }).sort((a,b)=>{
    if(sort==="composite")return(b.composite_score||0)-(a.composite_score||0);
    if(sort==="conservation")return(b.score_conservation||0)-(a.score_conservation||0);
    if(sort==="venture")return(b.score_venture||0)-(a.score_venture||0);
    if(sort==="trl")return(b.trl_level||0)-(a.trl_level||0);
    return(a.accepted_name||"").localeCompare(b.accepted_name||"");
  });

  const famFiltered=species.filter(s=>{
    const ms=!famSearch||s.accepted_name?.toLowerCase().includes(famSearch.toLowerCase());
    const mf=famFilter==="all"||s.family===famFilter;
    return ms&&mf;
  });

  const grouped=families.reduce((acc,fam)=>{
    const members=famFiltered.filter(s=>s.family===fam);
  ...
  return<>
    ...
    <div style={{display:"flex",gap:6,marginBottom:12,alignItems:"center",flexWrap:"wrap"}}>
      <div style={{display:"flex",border:"1px solid #e8e6e1",borderRadius:8,overflow:"hidden",marginRight:8}}>
        {[{k:"score",l:"Score View"},{k:"family",l:"Family View"}].map(v=><button key={v.k} onClick={()=>setViewMode(v.k)} style={{padding:"6px 14px",fontSize:11,border:"none",borderRight:v.k==="score"?"1px solid #e8e6e1":"none",cursor:"pointer",background:viewMode===v.k?"#f4f3ef":"#fff",color:viewMode===v.k?"#2c2c2a":"#888",fontWeight:viewMode===v.k?600:400}}>{v.l}</button>)}
      </div>
      {viewMode==="score"&&<>
        <input type="text" placeholder="Search..." value={search} onChange={e=>setSearch(e.target.value)} style={{flex:"1 1 140px",minWidth:120,...S.input}}/>
        ...
        <select value={fD} onChange={e=>setFD(e.target.value)} style={S.input}><option value="all">All decisions</option>{decisions.map(d=><option key={d} value={d}>{d}</option>)}</select>
        <select value={sort} onChange={e=>setSort(e.target.value)} style={S.input}><option value="composite">Composite</option><option value="conservation">Conservation</option><option value="venture">Venture</option><option value="trl">TRL</option><option value="name">Name</option></select>
      </>}
      {viewMode==="family"&&<>
        <input type="text" placeholder="Tür ara..." value={famSearch} onChange={e=>setFamSearch(e.target.value)} style={{flex:"1 1 160px",minWidth:140,...S.input}}/>
        <select value={famFilter} onChange={e=>setFamFilter(e.target.value)} style={S.input}>
          <option value="all">Tüm familyalar</option>
          {families.map(f=><option key={f} value={f}>{f} ({species.filter(s=>s.family===f).length})</option>)}
        </select>
      </>}
    </div>

    {viewMode==="score"&&<>
      <p style={{...S.sub,margin:"0 0 8px"}}>{filtered.length}/{species.length} species</p>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))",gap:8}}>
        {filtered.map(sp=><SpeciesCard key={sp.id} sp={sp} expanded={exp===sp.id} onToggle={()=>setExp(exp===sp.id?null:sp.id)}/>)}
      </div>
    </>}

    {viewMode==="family"&&<>
      <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}>
        <div style={{background:"#f4f3ef",borderRadius:8,padding:"6px 12px",fontSize:12}}><span style={{color:"#999"}}>Gösterilen: </span><strong>{famFiltered.length}</strong></div>
        <div style={{background:"#f4f3ef",borderRadius:8,padding:"6px 12px",fontSize:12}}><span style={{color:"#999"}}>Familya: </span><strong>{Object.keys(grouped).length}</strong></div>
      </div>
      {Object.entries(grouped).map(([fam,spp],i)=><FamilyGroup key={fam} family={fam} species={spp} defaultOpen={i<3}/>)}
      {famFiltered.length===0&&<p style={{textAlign:"center",padding:40,color:"#999"}}>Sonuç bulunamadı</p>}
    </>}
  </>
}
...
/* ─── PUBLICATIONS VIEW ─── */
function PublicationsView({publications}){const[search,setSearch]=useState("");const[expanded,setExpanded]=useState(null);const filtered=publications.filter(p=>{if(!search)return true;const s=search.toLowerCase();return(p.title||"").toLowerCase().includes(s)||(p.authors||"").toLowerCase().includes(s)||(p.journal||"").toLowerCase().includes(s)||(p.species?.accepted_name||"").toLowerCase().includes(s)});const sorted=[...filtered].sort((a,b)=>(b.year||0)-(a.year||0));const oaCount=publications.filter(p=>p.open_access).length;return<div><input type="text" placeholder="Search title, author, journal, or species..." value={search} onChange={e=>setSearch(e.target.value)} style={{width:"100%",marginBottom:12,...S.input}}/><div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}>{[{l:"Total publications",v:publications.length},{l:"Open access",v:oaCount},{l:"Species covered",v:[...new Set(publications.map(p=>p.species_id).filter(Boolean))].length},{l:"Sources",v:[...new Set(publications.map(p=>p.source).filter(Boolean))].length}].map(s=><div key={s.l} style={{flex:"1 1 100px",...S.metric}}><div style={S.mLabel}>{s.l}</div><div style={S.mVal()}>{s.v}</div></div>)}</div><p style={S.sub}>{sorted.length} publications · Sorted by year</p><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))",gap:8}}>{sorted.slice(0,50).map(p=><div key={p.id} onClick={()=>setExpanded(expanded===p.id?null:p.id)} style={{...S.card,padding:14,cursor:"pointer",border:expanded===p.id?"2px solid #85B7EB":"1px solid #e8e6e1"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,marginBottom:4}}><div style={{flex:1}}><div style={{fontSize:12,fontWeight:600,color:"#2c2c2a",lineHeight:1.4}}>{(p.title||"Untitled").slice(0,120)}{(p.title||"").length>120?"...":""}</div></div><div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:2,flexShrink:0}}>{p.year&&<Pill color="#0C447C" bg="#E6F1FB">{p.year}</Pill>}{p.open_access&&<Pill color="#085041" bg="#E1F5EE">OA</Pill>}</div></div><div style={{fontSize:10,color:"#888",marginBottom:4}}>{(p.authors||"—").slice(0,80)}{(p.authors||"").length>80?"...":""}</div><div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{p.journal&&<Pill color="#3C3489" bg="#EEEDFE">{(p.journal||"").slice(0,30)}</Pill>}{p.species?.accepted_name&&<Pill color="#085041" bg="#E1F5EE">{p.species.accepted_name}</Pill>}{p.primary_topic&&<Pill color="#854F0B" bg="#FAEEDA">{(p.primary_topic||"").slice(0,25)}</Pill>}</div>{expanded===p.id&&<div style={{marginTop:10,paddingTop:10,borderTop:"1px solid #e8e6e1",fontSize:11}}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"6px 14px"}}>{[{l:"DOI",v:p.doi?p.doi.replace("https://doi.org/",""):""},{l:"Source",v:p.source},{l:"Topic",v:p.primary_topic},{l:"Relevance",v:p.relevance_score?`${Math.round(p.relevance_score*100)}%`:"—"}].map(({l,v})=><div key={l}><span style={{color:"#b4b2a9",fontSize:9}}>{l}</span><div style={{color:"#2c2c2a"}}>{v||"—"}</div></div>)}</div>{p.doi&&<a href={p.doi} target="_blank" rel="noopener noreferrer" style={{display:"inline-block",marginTop:6,fontSize:10,color:"#185FA5",textDecoration:"none"}}>View paper ↗</a>}</div>}</div>)}</div>{sorted.length>50&&<p style={{...S.sub,textAlign:"center",marginTop:12}}>Showing 50 of {sorted.length} publications</p>}</div>;}
  ...
  return<div style={{display:"flex",minHeight:"100vh",background:"#f8f7f4"}}>
    ...
    <div style={{width:side?220:0,flexShrink:0,overflow:"hidden",background:"#fff",borderRight:"1px solid #e8e6e1",transition:"width 0.25s ease",display:"flex",flexDirection:"column"}}>
      <div style={{padding:"18px 14px 14px",flex:1,overflow:"hidden"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:20}}>
          <div style={{width:32,height:32,borderRadius:8,background:"linear-gradient(145deg,#085041,#1D9E75)",display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{color:"#fff",fontSize:14,fontWeight:700,fontFamily:"Georgia,serif"}}>A</span></div>
          <div><div style={{fontSize:14,fontWeight:700,letterSpacing:-0.5,color:"#2c2c2a",fontFamily:"Georgia,serif"}}>ATLAS</div><div style={{fontSize:7,color:"#b4b2a9",letterSpacing:1.5,textTransform:"uppercase"}}>GEOCON v2.4</div></div>
        </div>
        ...
        <div style={{marginTop:12,padding:10,background:"#f4f3ef",borderRadius:8,fontSize:9,color:"#888",lineHeight:1.8}}>
          ...
          <div><strong style={{color:"#2c2c2a"}}>{species.length}</strong> species · <strong style={{color:"#2c2c2a"}}>{metabolites.length}</strong> cpds</div>
          <div><strong style={{color:"#2c2c2a"}}>{publications.length}</strong> pubs · <strong style={{color:"#2c2c2a"}}>{researchers.length}</strong> res.</div>
        </div>
      </div>
      {/* SIDEBAR FOOTER — tek blok */}
      <div style={{padding:14,borderTop:"1px solid #e8e6e1"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
          <div style={{width:26,height:26,borderRadius:6,background:role.color,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{color:"#fff",fontSize:10,fontWeight:600}}>{role.ic}</span></div>
          <div><div style={{fontSize:11,fontWeight:600,color:"#2c2c2a"}}>{user.name}</div><div style={{fontSize:8,color:"#b4b2a9"}}>{role.label}</div></div>
        </div>
    ...
    <div style={{flex:1,minWidth:0,padding:"16px 20px 28px",overflow:"auto"}}>
      ...
      <div style={{display:"flex",gap:6,marginBottom:16,flexWrap:"wrap"}}>
        {[{l:"Species",v:species.length,c:"#1D9E75"},{l:"Compounds",v:metabolites.length,c:"#534AB7"},{l:"Publications",v:publications.length,c:"#185FA5"},{l:"Researchers",v:researchers.length,c:"#D85A30"},{l:"Threatened",v:threatened,c:"#E24B4A"}].map(s=><div key={s.l} style={{flex:"1 1 100px",...S.card,padding:"10px 14px",border:"1px solid #e8e6e1"}}><div style={S.mLabel}>{s.l}</div><div style={S.mVal(s.c)}>{s.v}</div></div>)}
      </div>

      {view==="species"&&<SpeciesModule species={species} exp={exp} setExp={setExp}/>}
      ...
      <div style={{marginTop:32,paddingTop:10,borderTop:"1px solid #e8e6e1",display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:4,fontSize:8,color:"#b4b2a9"}}>
        <span>GEOCON ATLAS v2.4 · {species.length} species · {publications.length} pubs · {researchers.length} researchers</span>
