...
- **VNEXT-ENDEMISM-VERTICAL.md** `[strategy · 11-agent]` — answers the founder endemism question: endemism is the right AXIS but a cross-cutting DIMENSION (an endemic_to relation, a public X/Y gear on every gearbox + a globe layer), NOT a separate vertical (7,130 geophytes are already endemics in GEOCON; vertical_id is single-valued — a separate vertical detonates every RPC). The broad non-geophyte vertical (rec. name TOPOCON) is real but NOT endemism-defined. Country orgs + invited foreigners = a SOCIAL layer (region-scoped programs + join door + steward/guest), the sovereignty hook = the cold-start recruit magnet. Multi-facet species (thyme/lavender) = one gearbox, conservation structurally first, value-gear a TRUE LOCK when wild-harvest is the threat. Wedge: West-Taurus -> Irano-Anatolian. Metric: local researcher x own endemic x receipt.
- **VNEXT-GEAR-MACHINE.md** `[strategy · the systematic+story]` — the gear/interlocking-domains model, inferred BOTTOM-UP from real geophyte science (Galanthus phylogeny, Colchicum new-species, saffron autotriploid, Drimia typification, etc.): a species page is a GEARBOX, a gear is a domain of work, a program is a team turning a gear, and gears MESH through real scientific prerequisites (one gear settled output is the tooth that turns the next). Seven gears in two layers (foundational X/Y: identity, safeguard, propagation, chemistry; value Z: ethnobotany, bioactivity-into-Exchange). Gives the felt STORY the platform lacked + how a program starts (5 steps on Galanthus). Founder-call open questions on gear granularity + the value-gear floor.
- **VNEXT-R3-DECISIONS.md** `[strategy · decision substrate]` — deep, no-shortcuts analysis of the round-3 founder decisions (4 domain experts): reintroduction gate-vs-advisory + the provisional state + which conservation actions truly gate (Godefroid/CPC/IUCN-grounded); Z value-honesty (tier floors, broadcast width, external-edge floor); ABS reserved-share + foundational equity (Nagoya/spinout-grounded); Y non-coercion + tic_edge authorship + the bench-arc-redux risk. Each: why-it-matters, full options (pros/cons/2nd-order), grounding, the tension, a one-input rec (NOT the answer). For the founder to analyze each deeply.
- **VNEXT-ROUND2-DESIGN.md** `[strategy · round 2]` — the locked v-next made buildable + SIMPLE: the whole 4-part reconception (program lifecycle + receipt-lineage rights + tic-architecture + first environment) adds ~ONE new table (positions) + 5 thin RPCs + 2 branches, because receipts-on-species, free-text program status, the Venn engine and the IUCN editor all already ship. First environment = the IUCN Red List Assessment bench (~70% shipped, real conservation value, attacks the 93% Not-Evaluated gap). Smallest first slice = a ONE-RPC edit (publish-weld -> verb -> receipt). Round-3 founder decisions listed.
