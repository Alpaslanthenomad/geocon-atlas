...
export async function fetchProgramMembers(programId) {
  const { data, error } = await supabase
    ...
    .select("*, researchers(id,name,affiliation,country,expertise_area,member_status)")
    .eq("program_id", programId);
...
export async function fetchProgramPublications(programId) {
  const { data, error } = await supabase
    ...
    .select("*, publications(id,title,authors,year,journal,doi,abstract,is_curated,category,first_author)")
    .eq("program_id", programId)
...
export async function initProgram({ species, whyNow, firstAction, ownerName }) {
  const defaultOwner = ownerName || "Alpaslan Acar";
