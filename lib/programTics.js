...
/**
 * Record a non-completing status (failure as data) on a tic — one of
 * 'blocked' | 'attempted_failed' | 'replaced_by_alternative', with an optional
 * money/PII-blind note. Owner OR tic assignee. Does NOT count toward progress.
 */
export async function setTicStatus(programId, ticId, status, note = null) {
  if (!programId || !ticId) return { success: false, error: "missing_params" };
  const { data, error } = await supabase.rpc("set_program_tic_status", {
    p_program_id: programId,
    p_tic_id: ticId,
    p_status: status,
    p_note: note,
  ...
  if (error) {
    console.warn("[programTics] setTicStatus error:", error.message);
    return { success: false, error: error.message };
  }
  return data || { success: false, error: "no_response" };
}
...
export const TIC_STATUS_LABEL = {
  ...
  replaced_by_alternative: "Replaced by alternative",
};
...
export const TIC_STATUS_COLOR = {
  ...
  blocked: { bg: "#FFEDD5", color: "#9A3412", border: "#FED7AA" },
  attempted_failed: { bg: "#FCEBEB", color: "#A32D2D", border: "#F7C1C1" },
  replaced_by_alternative: { bg: "#F1EFE8", color: "#5F5E5A", border: "#D3D1C7" },
};
