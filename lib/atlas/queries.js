...
import { getActiveVerticalId } from "./activeVertical";
...
export async function fetchSpeciesPage({
  ...
  // GATE-0: scope the browse to the viewer's active vertical (default geophytes)
  // unless a caller explicitly overrides (e.g. an admin cross-vertical view).
  const vid = verticalId || (await getActiveVerticalId());
...
export async function fetchMetabolitesForSpecies(speciesId) {
  ...
  const { data, error } = await supabase
    .from("metabolites")
    .select("id, compound_name, compound_class, cas_number, reported_activity, confidence, plant_organ, source, created_by")
    .eq("species_id", speciesId)
...
/** Distinct families currently in the atlas, for the filter sidebar + the
 *  family-first browse on the species route. Goes through the SQL aggregate
 *  RPC so we don't get capped at Supabase's 1000-row REST default. */
export async function fetchAtlasFamilies(verticalId) {
  // GATE-0: scope family counts to the active vertical (default geophytes).
  const vid = verticalId || (await getActiveVerticalId());
  const { data, error } = await supabase.rpc("get_atlas_family_counts", { p_vertical_id: vid });
  if (error) {
    console.warn("[atlas] families fetch error:", error.message);
    return [];
  }
